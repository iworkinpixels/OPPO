    ...
    def sAmp(self, time):
        ...
        if l > 0:
            ...
            if l > self.a and l < (self.a + self.d):                        # We are in the decay phase
                ...
                amp = (1-factor) * (1-self.s) + self.s                      # Reduce the amplitude to the sustain amplitude over the course of the decay time 

            if l > (self.a+self.d):                                         # We are in the sustain phase
                amp = self.s                                                # Maintain the sustain amplitude until the note is released
            
            if time > self.note_off and l < self.note_off + self.r:            # Note has been released, so we are in the release phase
                factor = (time - self.note_off) / self.r
                amp = self.s - (factor * self.s)                            # Reduce the amplitude to 0 over the course of the release time
            if l >= self.note_off + self.r:
                amp = 0.0
...
class Operator:
    ...
    def expValue(self, sinval):
        return (self.explut[255-(sinval&0xFF)]+1024) >> (sinval>>8)
